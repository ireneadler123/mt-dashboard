...
###########################################################################################################################################################################
    ...
    with middle:
        if kind == 'QTD - YOY %':
            ...
            df_past = df[(df['Ngày lấy đơn'].astype('datetime64[ns]') >= (date1 - timedelta(days = 365))) & (pd.to_datetime(df['Ngày lấy đơn'].astype('datetime64[ns]'), dayfirst = True) <= (date2 - timedelta(days = 365)))]
